...
merge datasets and save total litter for mr trash wheel for inline
printing
...
tons. The total number of cigarette butts collected by gywnnda trash
...
The `zori_zip` dataset contains 17516 observations of 8 variables
including observed values of the Zillow Observed Rent Index (ZORI)
across different counties, bouoroughs, and neighborhoods of NYC between
January 2015 and August 2024. There are 149 unique zip codes and 43
unique neighborhoods in the dataset.

The number of ZIP codes that appear in the ZIP code dataset but not in
the Zillow ZORI dataset is 171. For example, some of these excluded ZIP
codes are:  
10464, 10474, 10475, 10499, 10550.

These zipcodes may be excluded because the zillow data only focuses on
the rental market so zip codes representing areas with a small rental
market, areas that are typically commercial or office space
(e.g. `10090 (World Trade Center)` ), or zipcodes in special-purpose
zones such as industrial (e.g. `11251 (Brooklyn Navy Yard)`) zones would
not be included in the zillow rental data.
