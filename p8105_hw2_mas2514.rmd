...
__This chunk imports the `pols_month.csv` dataset from FiveThirtyEight and creates a clean the `pols_month_df` dataframe__
...
#
__This chunk imports the `snp.csv` dataset from FiveThirtyEight, and creates a clean `snp_df` dataframe__
...
#
__This chunk imports the `unemployment.csv` dataset and creates a clean `unemployment_df` dataframe__
...
#
__This chunk joins the datasets `pols_month_df` and `snp_df`, and merges `unemployment_df` with the result__
...
__This chunk reads the `Mr. Trash Wheel` dataset and creates a clean `mr_trash_df` dataframe__
...
#
__This chunk reads the `Professor Trash Wheel` data and creates clean`prof_trash_df` dataframe__
...
#
__This chunk reads the `Gwynns Falls Trash Wheel` data and creates clean `gywnnda_trash_df` dataframe__
...
#
__This chunk merges `mr_trash_df`, `prof_trash_df` and `gwynnda_trash_df` into a single clean dataframe `trash_wheel_df`__
...
* `trash_wheel` - the specific water-wheel vessel that collected the litter (mr, prof, gwynnda)
...
The total number of cigarette butts collected by gwynnda trash wheel in June of 2022 was `18,120`.


## Problem 3 

__This chunk reads the `Zillow Observed Rent Index (ZORI)` data for NYC from January 2015 and August 2024 and creates a clean `nyc_zori_df` dataframe__
...
__This chunk reads the `Zip Codes` data and creates a clean `nyc_zipcodes_df` datafame__
...
__This chunk merges the `zori_df` and `nyc_zipcodes_df` dataframes to one clean `nyc_rent_index_df`__
...
The `nyc_rent_index` dataset contains 17516 observations of 8 variables including observed values of the Zillow Observed Rent Index (ZORI) across different counties, boroughs, and neighborhoods of NYC between January 2015 and August 2024. There are '`r dplyr::n_distinct(nyc_rent_index_df$zip_code)`' unique zip codes and '`r dplyr::n_distinct(nyc_rent_index_df$neighborhood)`' unique neighborhoods in the dataset. 

The number of zipcodes that appear in the `nyc_zipcodes` dataset but not in the `nyc_zori` dataset is `r nrow(anti_join(nyc_zipcodes_df, nyc_zori_df, by = "zip_code"))`. For example, some of these excluded ZIP codes are:  
`r paste(head(anti_join(nyc_zipcodes_df, nyc_zori_df, by = "zip_code")$zip_code, 5), collapse = ", ")`. 
Since the Zillow Observed Rent Index (ZORI) only measures the observed market rate rent, certain zipcodes may be excluded when there is not a sufficient rental market. This could include areas that are typically majority commercial or office space (e.g. `10090 (World Trade Center)`), special-purpose zones such as industrial zones (e.g. `11251 (Brooklyn Navy Yard)`), or areas with small rental markets. 
